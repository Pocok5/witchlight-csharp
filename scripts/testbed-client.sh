...
HOST="${WITCHLIGHT_HOST:-localhost:$TESTBED_PORT}"
MODE="${1:-connect}"
...
case "$MODE" in
	...
	solo)    args=(--openWorld witchlight) ;;
	menu)    args=() ;;
...
mkdir -p "$CLIENT_DATA"
sync_settings
install_mod "$CLIENT_DATA" --target client
install_extra_mods "$CLIENT_DATA"

case "$MODE" in
	connect) printf 'connect: %s\n' "$HOST" ;;
	solo)    printf 'world:  local "witchlight"\n' ;;
	menu)    printf 'start:  main menu\n' ;;
esac

printf 'data:   %s\n' "$CLIENT_DATA"
# Said here as well as by the server script, because connecting is the common
# case and the map is the thing being tested.
